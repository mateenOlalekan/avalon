...
import { Search, ShoppingCart, Truck, Info, User, ChevronDown, UserRoundCheck, Menu, X, Bell, ShoppingBag, Package, Heart, LogOut } from 'lucide-react';
...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (mobileMenuRef.current && isMenuOpen && !mobileMenuRef.current.contains(event.target)) {
        setIsMenuOpen(false);
      }
    };
            ...
            <div className="hidden md:flex items-center space-x-6">
              <div className="relative" ref={userDropdownRef}>
                ...
                >
                  {currentUser ? (
                    <>
                      <UserRoundCheck size={22} className="text-green-600" />
                    ...
                    {currentUser ? (
                      ...
                      <>
                        ...
                        <div className="text-center text-xs text-gray-500 mb-2">
                          New customer? <Link to="/register" className="text-green-600 hover:underline">Register here</Link>
                        </div>
                      </>
                    )}
                  </div>
              ...
              <Link to="/about" className="flex items-center text-gray-700 hover:text-green-600 transition-colors duration-200" aria-label="About us">
                <Info size={20} className="hover:scale-110 transition-transform duration-200" />
              </Link>

              <Link to="/cart" className="flex items-center text-gray-700 hover:text-green-600 group relative transition-colors duration-200" aria-label="Shopping cart">
                ...
                {currentUser ? (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="text-center text-xs text-gray-500 mt-1">
                      New customer? <Link to="/register" onClick={() => setIsMenuOpen(false)} className="text-green-600 hover:underline">Register here</Link>
                    </div>
                  </div>
                )}
              </div>
                      ...
                      {openMobileCategory === category.name && (
                        <ul className="bg-gray-50 pl-8 py-1">
                          {category.subcategories.map((subcat) => (
                            <li key={subcat}>
                              <Link 
                                to={`/category/${category.name.toLowerCase()}/${subcat.toLowerCase().replace(/\s+/g, '-')}`}
